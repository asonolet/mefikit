...
pub fn in_sphere(x: &[f64; 3], center: &[f64; 3], r: f64) -> bool {
    ...
    let pa = ro::Coord3D {x: center[0] + r, y: center[1], z: center[2]};
    let pb = ro::Coord3D {x: center[0], y: center[1] + r, z: center[2]};
    let pc = ro::Coord3D {x: center[0] - r, y: center[1], z: center[2]};
    let pd = ro::Coord3D {x: center[0], y: center[1], z: center[2] + r};
    ro::insphere(pa, pb, pc, pd, x) > 0.0
}

pub fn in_circle(x: &[f64; 2], center: &[f64; 2], r: f64) -> bool {
    let x = ro::Coord {x: x[0], y: x[1]};
    let pa = ro::Coord {x: center[0] + r, y: center[1]};
    let pb = ro::Coord {x: center[0], y: center[1] + r};
    let pc = ro::Coord {x: center[0] - r, y: center[1]};
